...
const BusinessSourcetype = () => {
    ...
    const searchParam = ["BusinessSourceTypeName", "isActive"];
    ...
    const validateForm = () => {
        const { BusinessSourceTypeName } = formValues;
        const errors = {};
        let isValid = true;

        if (!BusinessSourceTypeName) {
            isValid = false;
            errors.BusinessSourceTypeName = "Business Source type name is required.";
        } else if (!/^[a-zA-Z ]+$/.test(BusinessSourceTypeName)) {
            errors.BusinessSourceTypeName = 'Name must contain only letters';
            isValid = false;
        }
    ...
    const handleEditClick = (row) => {
        ...
        setFormValues({
            ...
            BusinessSourceTypeName: row.BusinessSourceTypeName,
            isActive: row.isActive
        });
        setShow(true);
    };

    const handleDeleteClick = (BusinessSourceTypeId, BusinessSourceTypeName) => {
        setToDelete({ id: BusinessSourceTypeId, name: BusinessSourceTypeName });
        setConfirmOpen(true);
    };
    ...
    const confirmDelete = () => {
        ...
        setLoading(true);
        api.delete(`/businesssourcetype/${toDelete.id}`)
            .then((res) => {
                toast.success(res.data.successMessage || "Business source type deleted successfully!");
                fetchBusinessSourceTypeData();
            })
            .catch((error) => {
    ...
    const columns = [
        {
            name: <h5>Business Source Type Name</h5>,
            selector: (row) => row.BusinessSourceTypeName,
            sortable: true,
        },
        {
            name: <h5>Status</h5>,
        ...
        },
        {
            ...
            cell: (row) => (
                <>
                    ...
                    <Link className="action-icon" onClick={() => handleDeleteClick(row.BusinessSourceTypeId, row.BusinessSourceTypeName)} title='Delete'>
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            BusinessSourceTypeName: formValues.BusinessSourceTypeName,
        ...
        try {
            let res;
            if (isEditMode) {
                res = await api.put(`/businesssourcetype/${formValues.BusinessSourceTypeId}`, payload);
            } else {
                res = await api.post("/businesssourcetype", payload);
            }
            handleClose();
            fetchBusinessSourceTypeData();
                ...
                <div className='d-flex'>
                    <div className='p-3' style={{ width: "100vw" }}>
                        <DataTable
                            className='DataTable'
                            columns={columns}
                            data={DataTableSettings.filterItems(busiSourceTypeData, searchParam, filterText)}
                            pagination
                ...
                <Offcanvas.Header closeButton>
                    <div className="w-100 text-center">
                        <Offcanvas.Title style={{ fontSize: "30px", fontWeight: 600 }}>
                            {isEditMode ? "Edit Business Source Type" : "Add Business Source Type"}
                        </Offcanvas.Title>
                    </div>
                </Offcanvas.Header>
                <Offcanvas.Body style={{ marginTop: "-2vh" }}>
                    <Form className='h-90' onSubmit={handleSubmit}>
                        <InputGroup className="mb-4">
                            <InputGroup.Text id='BusinessSourceTypeName'>
                                <MdOutlinePersonOutline size={25} color='#ffc800' />
                            </InputGroup.Text>
                            <Form.Control
                                name="BusinessSourceTypeName"
                                value={formValues.BusinessSourceTypeName}
                                onChange={(e) => handleChange("BusinessSourceTypeName", e.target.value.replace(/[^a-zA-Z ]/g, ""))}
                                placeholder="Business source type name"
                                isInvalid={!!errors.BusinessSourceTypeName}
                                isValid={formValues.BusinessSourceTypeName && !errors.BusinessSourceTypeName}
                            />
                            {errors.BusinessSourceTypeName && <span className="error-msg">{errors.BusinessSourceTypeName}</span>}
                        </InputGroup>
...
export default BusinessSourcetype
