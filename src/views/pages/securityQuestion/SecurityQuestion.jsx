...
const SecurityQuestion = () => {
    ...
    const searchParam = ["securityQuestionName", "isActive"];
    ...
    const fetchSecurityData = async () => {
        setLoading(true);
        try {
            const res = await api.get(`/securityquestion`);
    ...
    const validateForm = () => {
        const { securityQuestionName } = formValues;
        const errors = {};
        let isValid = true;

        if (!securityQuestionName) {
            isValid = false;
            errors.securityQuestionName = "Security question name is required.";
        } else if (!/^[a-zA-Z ]+$/.test(securityQuestionName)) {
            errors.securityQuestionName = 'Name must contain only letters';
            isValid = false;
        }
    ...
    const handleEditClick = (row) => {
        ...
        setFormValues({
            ...
            securityQuestionName: row.securityQuestionName,
            isActive: row.isActive
        });
        setShow(true);
    };

    const handleDeleteClick = (securityQuestionID, securityQuestionName) => {
        setToDelete({ id: securityQuestionID, name: securityQuestionName });
        setConfirmOpen(true);
    };
    ...
    const confirmDelete = () => {
        ...
        setLoading(true);
        api.delete(`/securityquestion/${toDelete.id}`)
            .then((res) => {
                toast.success(res.data.successMessage || "Security question deleted successfully!");
                fetchSecurityData();
            })
            .catch((error) => {
    ...
    const columns = [
        {
            name: <h5>Securty Question</h5>,
            selector: (row) => row.securityQuestionName,
            sortable: true,
        },
        {
            name: <h5>Status</h5>,
        ...
        },
        {
            ...
            cell: (row) => (
                <>
                    ...
                    <Link className="action-icon" onClick={() => handleDeleteClick(row.securityQuestionID, row.securityQuestionName)} title='Delete'>
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            securityQuestionName: formValues.securityQuestionName,
        ...
        try {
            let res;
            if (isEditMode) {
                res = await api.put(`/securityquestion/${formValues.securityQuestionID}`, payload);
            } else {
                res = await api.post("/securityquestion", payload);
            }
            handleClose();
            fetchSecurityData();
                ...
                <div className='d-flex'>
                    <div className='p-3' style={{ width: "100vw" }}>
                        <DataTable
                            className='DataTable'
                            columns={columns}
                            data={DataTableSettings.filterItems(securityData, searchParam, filterText)}
                            pagination
                ...
                <Offcanvas.Header closeButton>
                    <div className="w-100 text-center">
                        <Offcanvas.Title style={{ fontSize: "30px", fontWeight: 600 }}>
                            {isEditMode ? "Edit Security Question" : "Add Security Question"}
                        </Offcanvas.Title>
                    </div>
                </Offcanvas.Header>
                        ...
                        <Row className='mb-2'>
                            <Col md={8}>
                                <InputGroup className="mb-4">
                                    <InputGroup.Text id='securityQuestionName'>
                                        <FaQuestion size={25} color='#ffc800' title='Question' />
                                    </InputGroup.Text>
                                    <Form.Control
                                        name="securityQuestionName"
                                        ref={commonRef}
                                        value={formValues.securityQuestionName}
                                        onChange={(e) => handleChange("securityQuestionName", e.target.value)}
                                        placeholder="Security Question"
                                        isInvalid={!!errors.securityQuestionName}
                                        isValid={formValues.securityQuestionName && !errors.securityQuestionName}
                                        autoComplete='off'
                                    />
                                    {errors.securityQuestionName && <span className="error-msg">{errors.securityQuestionName}</span>}
                                </InputGroup>
                            </Col>
                            <Col md={4}>
